...
class CustomHelp extends Help {
	/**
	 * Ensure all commands and subcommands have a help option by default.
	 * This is called recursively from gatherCommands after all commands are registered.
	...
	static ensureHelpOption(cmd) {
		...
		if (cmd.commands && cmd.commands.length) {
			for (const sub of cmd.commands) {
				CustomHelp.ensureHelpOption(sub);
			}
		}
	}
	/**
...
export async function gatherCommands(program) {
	...
	// Set CustomHelp, showHelpAfterError, and helpOption for all subcommands recursively
